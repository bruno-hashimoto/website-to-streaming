<?php
  ...
  $categorias = $sql->select('select nome,id from `sh_categoria` where status = 1 and excluido = 0 order by nome asc');
?>
    ...
    <div id="inserir">
      <form action="" method="post" class="validaFormulario" enctype="multipart/form-data" id="formAlterar">
        ...
        <div>
           ...
           <a href="<?php echo $_SERVER['REQUEST_URI'];?>" id="IDdoBotao">Excluir Imagem</a>
        </div>
        <div>
...
<!--
      ...
      <select name="input[id_categoria]" id="categorias">
        <option value="">Selecione uma opção</option>
        <?php
          if( $categorias )  {
              foreach($categorias as $row)
              {
                  echo '<option value="'.$row['id'].'" '.($row['id'] == $ln['id_categoria'] ? 'selected="selected"':'').' >'.$row['nome'].'</option>';
              }
           }
        ?>
      </select>

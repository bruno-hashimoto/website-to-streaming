...
class GUMP
{
    ...
    public function get_errors_array($convert_to_string = null)
    {
        ...
        foreach($this->errors as $e) {
            ...
            $field_name = str_replace(array('_','-'), chr(32), $e['field']);
            $field = $e['field'];
            $param = $e['param'];

            $param_name = $e['param'];

            if ( is_string( $e['param'] ) ) {
                $param_name = str_replace(array('_','-'), chr(32), $e['param']);
            }
            ...
            switch($e['rule']) {
                case 'mismatch' :
                    $resp[$field] = "Não existe uma regra de validação para $field_name";
                    break;
                case 'validate_required':
                    $resp[$field] = "O campo $field_name é obrigatório";
                    break;
                case 'validate_valid_email':
                    $resp[$field] = "Informe uma email válido";
                    break;
                case 'validate_max_len':
                    if($param == 1) {
                        $resp[$field] = "O campo $field_name pode ter no máximo $param caracteres";
                    } else {
                        $resp[$field] = "O campo $field_name pode ter no máximo $param caracteres";
                    }
                    break;
                case 'validate_min_len':
                    if($param == 1) {
                        $resp[$field] = "O campo $field_name não pode ter menos que $param caracteres";
                    } else {
                        $resp[$field] = "O campo $field_name não pode ter menos que $param caracteres";
                    }
                    break;
                case 'validate_exact_len':
                    if($param == 1) {
                        $resp[$field] = "O campo $field_name precisa ter exatamente $param caracteres";
                    } else {
                        $resp[$field] = "O campo $field_name precisa ter exatamente $param caracteres";
                    }
                    break;
                case 'validate_alpha':
                    $resp[$field] = "O campo $field_name só pode conter letras";
                    break;
                case 'validate_alpha_numeric':
                    $resp[$field] = "O campo $field_name tem que ter letras e números";
                    break;
                case 'validate_alpha_dash':
                    $resp[$field] = "O campo $field_name tem que ter traço";
                    break;
                case 'validate_numeric':
                    $resp[$field] = "O campo $field_name precisa ser um número";
                    break;
                case 'validate_integer':
                    $resp[$field] = "O campo $field_name precisa ser um número inteiro";
                    break;
                case 'validate_boolean':
                    $resp[$field] = "O campo $field_name precisa conter um valor boleano";
                    break;
                case 'validate_float':
                    $resp[$field] = "O campo $field_name precisa ser um número real";
                    break;
                case 'validate_valid_url':
                    $resp[$field] = "Informe uma url válida";
                    break;
                case 'validate_url_exists':
                    $resp[$field] = "A url $field_name não existe";
                    break;
                case 'validate_valid_ip':
                    $resp[$field] = "IP inválido";
                    break;
                case 'validate_valid_cc':
                    $resp[$field] = "Informe um número válido";
                    break;
                case 'validate_valid_name':
                    $resp[$field] = "Informe um nome válido";
                    break;
                case 'validate_contains':
                    $resp[$field] = "O campo $field_name precisa conter um desses valores: ".implode(', ', $param);
                    break;
                case 'validate_street_address':
                    $resp[$field] = "Informe um endereço válido";
                    break;
                case 'validate_date':
                    $resp[$field] = "Informe uma data válida";
                    break;
                case 'validate_min_numeric':
                    $resp[$field] = "Informe um número maior ou igual a $param";
                    break;
                case 'validate_max_numeric':
                    $resp[$field] = "Informe um número menor ou igual a $param";
                    break;
                case 'validate_equals':
                    $resp[$field] = "O campo $field_name deve ser igual ao $param_name";
                    break;
                case 'validate_cpf':
                    $resp[$field] = "Informe um cpf válido";
                    break;
                case 'validate_cnpj':
                    $resp[$field] = "Informe um cnpj válido";
                    break;
                default:
                    $resp[$field] = "O campo $field_name é inválido";
            }
        }

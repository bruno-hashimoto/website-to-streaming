    ...
    class Banco
    {
        ...
        public function __construct($dBug = false)
        {
            try
            {
                ...
                $this->setChar('utf8');
                //$this->setTimeZone();
                register_shutdown_function("mysqli_close",self::$conn);
            }
            catch(Exception $e)
            {
        ...
        {
            self::$conn = new mysqli(self::HOST, self::LOGIN, self::SENHA, self::DATAB);
            // print_r(self::$conn); die;
            ...
            // if(!self::$conn)
            // {
            //     throw new Exception("Erro ao tentar conectar-se com o banco de dados &#187;  <b>".self::HOST."</b> - ".mysqli_error());
            // }
            // else if(!mysql_select_db(self::DATAB,self::$conn))
            // {
            //     throw new Exception("Erro ao tentar conectar-se com a base de dados &#187;  <b>".self::DATAB."</b> - ".mysqli_error());
            // }
        }
        ...
        final public function select($sql = NULL, $n = false)
        {
            try
            {
                ...
                {
                    throw new Exception("Query inválida &#187; <b>".$sql."</b> - ".mysqli_error());
                }
            }
            catch(Exception $e)
            {
            ...
            try
            {
                if(!empty($table))
                {
                    if(!empty($array))
                    {
                        ...
                        {
                            throw new Exception("Erro , ao tentar inserir &#187; <b>".$insert."</b> - ".mysqli_error());
                        }
                        else
                        {
                            return true;
            ...
            try {
                if(!empty($table)) {
                    if(!empty($array)) {
                        ...
                        {
                            throw new Exception("Erro , ao tentar atualizar &#187; <b>".$update."</b> - ".mysqli_error());
                        }
                        else
                        {
                            return true;
        ...
        final public function fetch($query, $n = false)
        {
            ...
            return  $n ? mysqli_fetch_row($query) : mysqli_fetch_assoc($query);
        }
        ...
        public static function error($msg = "")
        {
            ...
            '
                <div style="width:960px; display:table; margin:auto;border:solid 1px red; height:auto; background:#ffffff; border: solid 1px #999; margin-bottom:10px; position:absolute;top:10px; left:10px;">
                    ...
                    <b style="color:#F00; display:block;text-transform:uppercase;">Erro de Banco de Dados</b>
